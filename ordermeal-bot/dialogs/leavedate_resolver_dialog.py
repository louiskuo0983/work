...
class leaveDateResolverDialog(CancelAndHelpDialog):
    def __init__(self, dialog_id: str = None):
        super(leaveDateResolverDialog, self).__init__(
            dialog_id or leaveDateResolverDialog.__name__
        )

        self.add_dialog(
            DateTimePrompt(
                DateTimePrompt.__name__, leaveDateResolverDialog.datetime_prompt_validator
            )
        )
        self.add_dialog(
            WaterfallDialog(
    ...
    async def initial_step(
        ...
        prompt_msg_text = "On what date would you going to leave?"
