...
from .leavedate_resolver_dialog import leaveDateResolverDialog
...
class BookingDialog(CancelAndHelpDialog):
    def __init__(self, dialog_id: str = None):
        ...
        self.add_dialog(leaveDateResolverDialog(leaveDateResolverDialog.__name__))
        self.add_dialog(
            WaterfallDialog(
    ...
    async def leave_date_step(
        ...
        # Capture the results of the previous step
        return await step_context.begin_dialog(
            leaveDateResolverDialog.__name__, booking_details.leave_date
        )
    ...
    async def confirm_step(
        ...
        # Capture the results of the previous step
        booking_details.leave_date = step_context.result
        message_text = (
            f"Please confirm, I have you traveling to: { booking_details.destination } from: "
            f"{ booking_details.origin } on: { booking_details.travel_date} to: { booking_details.leave_date}."
        )
        prompt_message = MessageFactory.text(
            message_text, message_text, InputHints.expecting_input
        )
